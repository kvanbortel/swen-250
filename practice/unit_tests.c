...
int test()
{
    ...
    // Test make_string_type
    printf( "\n----------------------- Test the make_string_type function ----------------\n" ) ;

	// Test 26 -- 
	assert( make_string_type( "" ).length == 0,
		"make_string_type expected 0 with empty string" )
		? passcount++ : failcount++ ;

	// Test 27 -- 
	assert( make_string_type( "Hello" ).length == 5,
		"make_string_type expected 5 for string 'Hello' " )
		? passcount++ : failcount++ ;
